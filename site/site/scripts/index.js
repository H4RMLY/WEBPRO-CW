...
function addToWorkout(e){
    ...
    if (e.target.classList.contains("clicked")) {
        postWorkout(selectedExercise, selectedExerciseTime);
    } else {
...
async function postWorkout(exerciseName, exerciseTime) {
    const payload = { name: exerciseName, time: exerciseTime };
...
async function showInstructions(e) {
    ...
    const response = await fetch('instructions/' + e.target.dataset.id);
    if (response.ok) {
...
async function cycleWorkout(){
    const workout = await getWorkout();

    for (let i = 0; i < workout.length; i++) {
        workoutControl.current = workout[i];
        workoutControl.next = workout[i + 1]
        const x = await waitForTimer(workoutControl.current.time, 10)
        showCurrentWorkout();
    }
}

function waitForTimer(time, x){
    timer(time);
    return new Promise((resolve) => {
